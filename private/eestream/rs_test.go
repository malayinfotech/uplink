...
func TestRSRanger(t *testing.T) {
	...
	encKey := storx.Key(sha256.Sum256([]byte("the secret key")))
	var firstNonce storx.Nonce
	const stripesPerBlock = 2
	blockSize := stripesPerBlock * rs.StripeSize()
	encrypter, err := encryption.NewEncrypter(storx.EncAESGCM, &encKey, &firstNonce, blockSize)
	if err != nil {
		t.Fatal(err)
	}
	...
	decrypter, err := encryption.NewDecrypter(storx.EncAESGCM, &encKey, &firstNonce, blockSize)
	if err != nil {
		t.Fatal(err)
	}
