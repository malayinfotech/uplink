...
func (client *Client) Download(ctx context.Context, limit *pb.OrderLimit, piecePrivateKey storx.PiecePrivateKey, offset, size int64) (_ *Download, err error) {
		...
		if notYetReceived < client.orderStep {
			...
			if newAllocation > 0 {
				...
				err = func() error {
					...
					if client.client.NodeURL().NoiseInfo.Proto != storx.NoiseProto_Unset {
