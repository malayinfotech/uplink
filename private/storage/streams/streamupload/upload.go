...
import (
	...
	"uplink/private/storage/streams/batchaggregator"
	"uplink/private/storage/streams/segmenttracker"
	"uplink/private/storage/streams/splitter"
	"uplink/private/storage/streams/streambatcher"
)
...
type EncryptedMetadata interface {
	...
	EncryptedMetadata(lastSegmentSize int64) (data []byte, encKey *storx.EncryptedPrivateKey, nonce *storx.Nonce, err error)
}
...
func UploadPart(ctx context.Context, segmentSource SegmentSource, segmentUploader SegmentUploader, miBatcher metaclient.Batcher, streamID storx.StreamID, eTagCh <-chan []byte) (_ Info, err error) {
	defer mon.Task()(&ctx)(&err)
	return uploadSegments(ctx, segmentSource, segmentUploader, miBatcher, nil, nil, streamID, eTagCh)
}

func uploadSegments(ctx context.Context, segmentSource SegmentSource, segmentUploader SegmentUploader, miBatcher metaclient.Batcher, beginObject *metaclient.BeginObjectParams, encMeta EncryptedMetadata, streamID storx.StreamID, eTagCh <-chan []byte) (_ Info, err error) {
	defer mon.Task()(&ctx)(&err)
...
func deleteCancelledObject(ctx context.Context, batcher metaclient.Batcher, beginObject *metaclient.BeginObjectParams, streamID storx.StreamID) (err error) {
	defer mon.Task()(&ctx)(&err)
