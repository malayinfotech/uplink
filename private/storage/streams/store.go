...
func NewStreamStore(metainfo *metaclient.Client, ec ecclient.Client, segmentSize int64, encStore *encryption.Store, encryptionParameters storx.EncryptionParameters, inlineThreshold, longTailMargin int) (*Store, error) {
	if segmentSize <= 0 {
...
func (s *Store) Put(ctx context.Context, bucket, unencryptedKey string, data io.Reader, metadata Metadata, expiration time.Time) (_ Meta, err error) {
	...
	for !eofReader.IsEOF() && !eofReader.HasError() {
		...
		if s.encryptionParameters.CipherSuite != storx.EncNull {
			segmentEncryption = metaclient.SegmentEncryption{
	...
	encryptedStreamInfo, err := encryption.Encrypt(streamInfo, s.encryptionParameters.CipherSuite, &contentKey, &storx.Nonce{})
	if err != nil {
		return Meta{}, errs.Wrap(err)
	}
...
func (s *Store) PutPart(ctx context.Context, bucket, unencryptedKey string, streamID storx.StreamID, partNumber uint32, eTagCh <-chan []byte, data io.Reader) (_ Part, err error) {
	defer mon.Task()(&ctx)(&err)

	var (
		currentSegment        uint32
		streamSize            int64
		lastSegmentContentKey storx.Key
...
func encryptETag(etag []byte, encryptionParameters storx.EncryptionParameters, contentKey *storx.Key) ([]byte, error) {
	...
	encryptedETag, err := encryption.Encrypt(etag, encryptionParameters.CipherSuite, etagKey, &storx.Nonce{})
	if err != nil {
		return nil, err
	}
...
// TODO move it to separate package?
func deriveETagKey(key *storx.Key) (*storx.Key, error) {
	return encryption.DeriveKey(key, "storx-etag-v1")
}
...
func deriveContentNonce(pos metaclient.SegmentPosition) (storx.Nonce, error) {
	// The increment by 1 is to avoid nonce reuse with the metadata encryption,
	// which is encrypted with the zero nonce.
	var n storx.Nonce
...
func decryptRanger(ctx context.Context, rr ranger.Ranger, plainSize int64, encryptionParameters storx.EncryptionParameters, derivedKey *storx.Key, encryptedKey storx.EncryptedPrivateKey, encryptedKeyNonce, startingNonce *storx.Nonce) (decrypted ranger.Ranger, err error) {
	defer mon.Task()(&ctx)(&err)
...
func (s *Store) deleteCancelledObject(ctx context.Context, bucketName, encryptedObjectKey string, streamID storx.StreamID) {
	var err error
	defer mon.Task()(&ctx)(&err)
...
func (s *Store) Ranger(ctx context.Context, response metaclient.DownloadSegmentWithRSResponse) (rr ranger.Ranger, err error) {
	...
	redundancy, err := eestream.NewRedundancyStrategyFromStorx(info.RedundancyScheme)
	if err != nil {
		return nil, err
	}
