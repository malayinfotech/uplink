...
func New(miBatcher metaclient.Batcher, streamID storx.StreamID) *Batcher {
	return &Batcher{
		miBatcher: miBatcher,
		streamID:  streamID,
	}
...
func (s *Batcher) StreamID() storx.StreamID {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.streamID
}
...
func creationDateFromStreamID(streamID storx.StreamID) (time.Time, error) {
	if streamID == nil {
