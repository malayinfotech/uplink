...
var (
	...
	encryptionParameters = storx.EncryptionParameters{CipherSuite: cipherSuite, BlockSize: 32}
	inlineThreshold      = 1024
	longTailMargin       = 1
	storxKey             = storx.Key{0: 1}
	expiration           = creationDate.Add(time.Hour)
	uploadInfo           = streamupload.Info{CreationDate: creationDate, PlainSize: 123}
	streamID             = storx.StreamID("STREAMID")
	partNumber           = int32(1)
	...
	testUpload := func(t *testing.T, uploadFn func(uploader *Uploader, c config) (*Upload, error)) {
		for _, tc := range []struct {
			...
			t.Run(tc.desc, func(t *testing.T) {
				...
				err := encStore.Add("BUCKET", unenc, enc, storxKey)
				require.NoError(t, err)
...
func TestEncryptedMetadata(t *testing.T) {
	...
	require.NoError(t, err)
	streamInfoBytes, err := encryption.Decrypt(streamMeta.EncryptedStreamInfo, e.cipherSuite, metadataKey, &storx.Nonce{})
	require.NoError(t, err)
...
func TestLimitsExchanger(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		segmentID, limits, err := e.ExchangeLimits(context.Background(), storx.SegmentID("IN"), []int{1, 2, 3})
		require.NoError(t, err)
		require.Equal(t, storx.SegmentID("OUT"), segmentID)
...
func (b fakeUploaderBackend) UploadPart(ctx context.Context, segmentSource streamupload.SegmentSource, segmentUploader streamupload.SegmentUploader, miBatcher metaclient.Batcher, streamIDIn storx.StreamID, eTagChIn <-chan []byte) (streamupload.Info, error) {
	if err := b.checkCommonParams(segmentSource, segmentUploader, miBatcher); err != nil {
		return streamupload.Info{}, err
	}
...
func (piecePutter) PutPiece(longTailCtx, uploadCtx context.Context, limit *pb.AddressedOrderLimit, privateKey storx.PiecePrivateKey, data io.ReadCloser) (hash *pb.PieceHash, deprecated *struct{}, err error) {
