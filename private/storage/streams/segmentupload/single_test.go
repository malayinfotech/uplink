...
var (
	...
	fakeSegmentID  = storx.SegmentID{0xFF}
	...
	fakeSegmentInfo = &splitter.SegmentInfo{
		Encryption:    metaclient.SegmentEncryption{EncryptedKeyNonce: storx.Nonce{0: 1}},
		PlainSize:     123,
		EncryptedSize: 456,
	}
)
...
type nodeKind storx.NodeID

func isNodeKind(nodeID storx.NodeID, kind nodeKind) bool {
...
func makeNodeID(i int, kind nodeKind) storx.NodeID {
	nodeID := storx.NodeID(kind)
	nodeID[1] = byte(i)
	return nodeID
}

func fastNodeID(i int) storx.NodeID {
	return makeNodeID(i, fastKind)
}

func makeNodeIDs(kinds ...nodeKind) []storx.NodeID {
	var nodes []storx.NodeID
...
func pieceID(num int) storx.PieceID {
	return storx.PieceID{0: byte(num)}
}
...
func (fakeLimitsExchanger) ExchangeLimits(ctx context.Context, segmentID storx.SegmentID, pieceNumbers []int) (storx.SegmentID, []*pb.AddressedOrderLimit, error) {
	return nil, nil, errors.New("exchanges disallowed for test")
}

type fakePiecePutter struct{}

func (fakePiecePutter) PutPiece(longTailCtx, uploadCtx context.Context, limit *pb.AddressedOrderLimit, privateKey storx.PiecePrivateKey, data io.ReadCloser) (hash *pb.PieceHash, deprecated *struct{}, err error) {
...
func mustNewPiecePrivateKey() storx.PiecePrivateKey {
	pk, err := storx.PiecePrivateKeyFromBytes(bytes.Repeat([]byte{1}, 64))
	if err != nil {
		panic(err)
	}
	return pk
...
func mustNewRedundancyStrategy() eestream.RedundancyStrategy {
	rs, err := eestream.NewRedundancyStrategyFromStorx(storx.RedundancyScheme{
