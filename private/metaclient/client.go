...
func newObjectInfo(object *pb.Object) RawObjectItem {
	...
	info := RawObjectItem{
		...
		EncryptionParameters: storx.EncryptionParameters{
			CipherSuite: storx.CipherSuite(object.EncryptionParameters.CipherSuite),
	...
	if pbRS != nil {
		info.RedundancyScheme = storx.RedundancyScheme{
			Algorithm:      storx.RedundancyAlgorithm(pbRS.Type),
...
func newListSegmentsResponse(response *pb.SegmentListResponse) ListSegmentsResponse {
	...
	ep := storx.EncryptionParameters{}
	if response.EncryptionParameters != nil {
		ep = storx.EncryptionParameters{
			CipherSuite: storx.CipherSuite(response.EncryptionParameters.CipherSuite),
...
func (client *Client) DownloadObject(ctx context.Context, params DownloadObjectParams) (_ DownloadObjectResponse, err error) {
	...
	if err != nil {
		if errs2.IsRPC(err, rpcstatus.NotFound) {
			return DownloadObjectResponse{}, storx.ErrObjectNotFound.Wrap(err)
		}
...
func newDownloadSegmentResponseWithRS(response *pb.SegmentDownloadResponse) DownloadSegmentWithRSResponse {
	...
	if response.RedundancyScheme != nil {
		info.RedundancyScheme = storx.RedundancyScheme{
			Algorithm:      storx.RedundancyAlgorithm(response.RedundancyScheme.Type),
