...
func (db *DB) DownloadObject(ctx context.Context, bucket, key string, options DownloadOptions) (info DownloadInfo, err error) {
	...
	if bucket == "" {
		return DownloadInfo{}, storx.ErrNoBucket.New("")
	}
	if key == "" {
		return DownloadInfo{}, storx.ErrNoPath.New("")
	}
...
// ObjectFromRawObjectItem converts RawObjectItem into storx.Object struct.
func (db *DB) ObjectFromRawObjectItem(ctx context.Context, bucket, key string, objectInfo RawObjectItem) (Object, error) {
	...
	if object.Stream.EncryptionParameters.CipherSuite == storx.EncUnspecified {
		object.Stream.EncryptionParameters = storx.EncryptionParameters{
			CipherSuite: storx.CipherSuite(streamMeta.EncryptionType),
...
func (db *DB) objectFromRawObjectListItem(bucket string, path storx.Path, listItem RawObjectListItem, stream *pb.StreamInfo, streamMeta pb.StreamMeta) (Object, error) {
	object := Object{
...
func (db *DB) typedDecryptStreamInfo(ctx context.Context, bucket string, unencryptedKey paths.Unencrypted,
	streamMetaBytes, metadataKey []byte, metadataNonce storx.Nonce) (_ *pb.StreamInfo, _ pb.StreamMeta, err error) {
	defer mon.Task()(&ctx)(&err)
	...
	cipher := storx.CipherSuite(streamMeta.EncryptionType)
	...
	streamInfo, err := encryption.Decrypt(streamMeta.EncryptedStreamInfo, cipher, contentKey, &storx.Nonce{})
	if err != nil {
		return nil, pb.StreamMeta{}, err
	}
...
func getEncryptedKeyAndNonce(metadataKey []byte, metadataNonce storx.Nonce, m *pb.SegmentMeta) (storx.EncryptedPrivateKey, *storx.Nonce) {
	if len(metadataKey) > 0 {
		return storx.EncryptedPrivateKey(metadataKey), &metadataNonce
	}
