...
// parseNodeURL parses the address into a storx.NodeURL adding the node id if necessary
// for known addresses.
func parseNodeURL(address string) (storx.NodeURL, error) {
	nodeURL, err := storx.ParseNodeURL(address)
	if err != nil {
		return nodeURL, packageError.Wrap(err)
	}

	// Node id is required in satelliteNodeID for all unknown (non-storx) satellites.
