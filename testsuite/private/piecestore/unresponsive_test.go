...
func TestUnresponsiveStoragenode(t *testing.T) {
	...
	storagenodeIdent, err := testidentity.PregeneratedIdentity(1, storx.LatestIDVersion())
	require.NoError(t, err)
	...
	uplinkIdent, err := testidentity.PregeneratedIdentity(1, storx.LatestIDVersion())
	require.NoError(t, err)
	...
	t.Run("Download", func(t *testing.T) {
		client, err := piecestore.Dial(ctx, uplinkDialer, storx.NodeURL{
		...
		_, privateKey, err := storx.NewPieceKey()
		require.NoError(t, err)
