...
const storxrelease = "v1.0.0" // uses uplink v1.0.0-rc.5.0.20200311190324-aee82d3f05aa
...
func TestOldUplink(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		...
		cmd := exec.Command("go", "install", "storx/cmd/uplink@"+storxrelease)
		cmd.Env = os.Environ()
...
func TestMove(t *testing.T) {
	...
	testplanet.Run(t, testplanet.Config{
		...
		cmd := exec.Command("go1.17.13", "install", "storx/cmd/uplink@v1.40.4")
		cmd.Env = os.Environ()
		cmd.Env = append(cmd.Env, "GOBIN="+ctx.Dir("binary"))
		output, err := cmd.CombinedOutput()
		t.Log(string(output))
